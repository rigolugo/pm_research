...
B0 Data API `/trades` mechanical-trust pilot artifacts. Current accepted state: B0 execution halted at `STOP_API_LOCAL_MISMATCH`, but the local-only diagnostic halted at `STOP_API_ARTIFACT_MISSING` because the run did not persist enough API/mismatch rows for offline temporal-overlap analysis. **This blocks B1 but does not cleanly distinguish API semantic mismatch from stale/incomplete local parquet.** No price series is persisted.

- `option_b_b0_manifest.json` — fixed 10-condition B0 manifest; this is the only confirmed persisted B0 file from the local artifact inspection.
- Expected-but-missing from the inspected artifact directory: reconciliation JSON, by-condition CSV, mismatch CSV, saved API rows, and saved mismatch rows. Because these are absent, offline overlap classifications (`NO_TEMPORAL_OVERLAP`, `OVERLAP_MATCHED`, `OVERLAP_API_LOCAL_MISMATCH`) cannot be determined from persisted artifacts.
...
- `SPEC_price_source_option_b_data_api_review.md` — **ACCEPTED**, at orchestrator review, with two doc-only wording amendments (§8 build-gate wording, §1.1 H2-hypothesis wording). A pessimistic, falsification-minded review of the Polymarket Data API `/trades` endpoint as a per-side/token-identity price-source candidate. Sets out H1 (local backfill incomplete, fixable) vs. H2 (trading genuinely one-sided, unfixable by any retrieval path), composite-identity reconciliation, pagination-completeness, typed numeric parsing, and identifier precision guards as spec-level requirements R1–R7; sets out a bounded two-phase B0/B1 test design (each phase separately authorized, no autonomous escalation); and restates the minimum typed-halt set. Current B0 state is recorded in `HANDOFF_orchestrator_option_b_b0_RESULT.md`: B0 halted at `STOP_API_LOCAL_MISMATCH`, then local-only failure diagnosis halted at `STOP_API_ARTIFACT_MISSING`. B0 did not establish Data API `/trades` mechanical trust and therefore blocks B1, but it does not cleanly distinguish API semantic mismatch from stale/incomplete local parquet. No B0 rerun, B1, full Pass 1, S2, P1/P2/P3, probe, scoring, backfill, wallet/OrdersMatched/`log_index`/PnL, or gate change is authorized. `named_binary_probe_blocked` stays `true`.
...
- `HANDOFF_orchestrator_option_b_b0_RESULT.md` — docs-only handoff recording the latest accepted B0 state: original authorized B0 execution halted at `STOP_API_LOCAL_MISMATCH`; local-only artifact inspection halted at `STOP_API_ARTIFACT_MISSING`; B1 not authorized; Option B not closed as a clean API semantic failure on this evidence.
- `SPEC_option_b_b0_failure_diagnostic.md` — **ACCEPTED / SPEC ONLY.** Pessimistic, falsification-minded spec-only design for a *possible* corrected B0 diagnostic (rerun not authorized by acceptance). Addresses the root defect (persist-before-halt ordering) and defines persistence requirements D1–D9 (full API/local/mismatch ledgers with temporal bounds, offline-recomputable classifications), time-bounded reconciliation safety rules with locked constants (`τ = 120 seconds`, `σ = 24 hours`), typed classification taxonomy with precedence, and the locked `OVERLAP_MATCHED` clean bar. Authorizes nothing further; a corrected run requires separate explicit second authorization. B1/P1/downstream remain blocked.
- `HANDOFF_orchestrator_option_b_b0_failure_diagnostic.md` — Claude-to-Orchestrator handoff recording the acceptance of `SPEC_option_b_b0_failure_diagnostic.md` and the four applied final spec locks: status line, locked constants (τ, σ, ledger cap), locked `OVERLAP_MATCHED` clean bar, and ledger-cap scope clarification. Documentation only; the corrected diagnostic run is not authorized.
...
- Active specs/handoffs as applicable — `SPEC_named_binary_probe.md` (spec only; post-S0 blocker addendum), `SPEC_price_source_s1_coverage.md` (accepted / spec only / coverage-only), `SPEC_price_source_alt_trade_prints.md` (accepted / historical after negative S1-ALT), `SPEC_price_source_option_b_data_api_review.md` (accepted spec context), `SPEC_option_b_b0_failure_diagnostic.md` (accepted / spec only / no rerun authorization), `HANDOFF_orchestrator_named_binary_probe_p0.md` (P0 accepted), `HANDOFF_orchestrator_named_binary_probe_p1_REVIEW.md` (P1 paused/blocked), `HANDOFF_orchestrator_option_b_spec_s1_1_patch.md`, `HANDOFF_orchestrator_option_b_b0_RESULT.md`, `HANDOFF_orchestrator_option_b_b0_failure_diagnostic.md` (spec locks applied).
