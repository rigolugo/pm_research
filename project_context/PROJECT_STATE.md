...
No C1B full sampled coverage, C2 reusable/production implementation, P1/P2/P3 continuation, probe execution, scoring, backfill, wallet discovery, OrdersMatched expansion, `log_index`, PnL, gate change, cap increase, row truncation, additional canary, SQL modification, or price artifact follows from the accepted C1A-F1 result.

Do not record `C1_CANARY_DESIGN_CLEAR` from the C1A-F1 run. The observed outcome is `C1_CANARY_EXECUTED_NEEDS_REVIEW`, and the evidence remains mixed.

The only further named-binary step, if explicitly authorized, must remain a separate price-source review/spec task under current guardrails. The accepted C1A-F1 result is reviewable mixed coverage/trust evidence only; it does not authorize C1B/C2/P1/P2/P3/probe execution, another canary, scoring, backfill, wallet discovery, OrdersMatched expansion, `log_index`, PnL, a price artifact, or a gate change.
...
Winners derive ONLY from payout numerators, never from price convergence. The named-binary probe remains NOT AUTHORIZED — a CLEAR* gate does not grant it. `SPEC_named_binary_probe.md` is ACCEPTED (spec only); its Stage P0 preflight is implemented and ACCEPTED (`P0_CLEAR`). Stage P1 remains BLOCKED on a proven price-input gap. The accepted C1A-F1 result is reviewable mixed coverage/trust evidence only; it does not authorize C1B/C2/P1/P2/P3/probe execution, another canary, scoring, backfill, wallet discovery, OrdersMatched expansion, `log_index`, PnL, a price artifact, or a gate change.
