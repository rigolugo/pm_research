...
- **S1 price-source coverage spec: ACCEPTED / SPEC ONLY.** `SPEC_price_source_s1_coverage.md` is the accepted, **coverage-only** plan for testing whether Polymarket CLOB `/prices-history` per token can cover the P0 universe (39,693 conditions) with a usable decision-time per-side price for both sides. It is **user-run only if later explicitly authorized** (on the Windows/Miniconda box; Claude does not fetch Polymarket data; no network-allowlist change is implied). **It does NOT unblock P1 by itself** — it only measures whether a candidate per-side price source is viable; a NEGATIVE result leaves P1 blocked with no `yes_price` fallback. Acceptance authorizes **nothing further**: no implementation, no S1 run, no network/data fetch, no S2, no P1/P2/P3, no probe execution, no scoring, no backfill. `named_binary_probe_blocked` stays `true`.

- **S1 Pass 1 sampled coverage: COMPLETED / ACCEPTED — RESULT: `S1_SOURCE_NOT_VIABLE`.** The S1 Pass 1 coverage run was implemented, patched through several data-shape fixes, then **user-run on the Windows box and accepted as a sampled coverage finding.** On the accepted run: sample 300/300; **248 valid-window** conditions measured, **52 invalid-window** excluded + reported; **496** side-token fetches; endpoint shape parsed cleanly (`GET /prices-history`, `history` list, point keys `p`/`t`, no deviation). **Level-B both-sides coverage (binding) cleared 0.95 in no subclass:** UP_DOWN 19/50 = 0.38, OVER_UNDER 51/98 ≈ 0.5204, NAMED_OTHER 65/100 = 0.65. Verdict **`S1_SOURCE_NOT_VIABLE`**. This is **Pass 1 sampled coverage only, not Pass 2 full-universe coverage.** CLOB `/prices-history` does **not** provide a usable decision-window per-side price for both sides across the sampled P0 universe. **Consequence: P1 remains BLOCKED with no `yes_price` fallback.** No Pass 2, S2, P1/P2/P3, probe, scoring, backfill, wallet/OrdersMatched/log_index/PnL, or gate change is authorized; `named_binary_probe_blocked` stays `true`. Artifacts: `artifacts/named_binary_probe/price_source_s1/price_source_s1_coverage.{json,md}` + `_by_condition.csv` + `_excluded.csv` + `_endpoint_shape.md`. Handoff: `HANDOFF_orchestrator_s1_pass1_RESULT.md`.

- **S1-ALT Pass 1 (Option A local trade-print) sampled coverage: COMPLETED / ACCEPTED — RESULT: `S1ALT_SOURCE_NOT_VIABLE`.** The first candidate alternative price source after the S1 negative — local trade-print reconstruction via `Store.load_trades()` (Option A, no network) — was implemented per `SPEC_price_source_alt_trade_prints.md` (approved, precision-loss corrected), patched through a review cycle, then **user-run on the Windows box and accepted as a sampled coverage finding.** On the accepted run: sample 300/300 reconstructed from the **exact accepted S1 Pass-1 sample** (248 measured-eligible + 52 S1-invalid-window, pre-excluded and never re-measured against local trades); measured_by_subclass UP_DOWN 50 / OVER_UNDER 98 / NAMED_OTHER 100 — identical to S1's breakdown. **Level-B both-sides coverage (binding) cleared 0.95 in no subclass:** UP_DOWN 13/50 = 0.26, OVER_UNDER 40/98 ≈ 0.4082, NAMED_OTHER 71/100 = 0.71. Verdict **`S1ALT_SOURCE_NOT_VIABLE`**. This is Pass 1 sampled coverage only; no price series persisted. **Consequence: local trade prints are not viable as a per-side decision-window price source on the sampled P0 universe either — P1 remains BLOCKED with no `yes_price` fallback and no `1 - price` synthesis.** No Pass 2, Option C, S2, P1/P2/P3, probe, scoring, backfill, wallet/OrdersMatched/log_index/PnL, or gate change is authorized; `named_binary_probe_blocked` stays `true`. Artifacts: `artifacts/named_binary_probe/price_source_s1_alt/price_source_s1_alt_coverage.{json,md}` + `_by_condition.csv` + `_excluded.csv` + `_source_shape.md`. Handoff: `HANDOFF_orchestrator_s1_alt_pass1_RESULT.md`.

- **Option B Data API `/trades` Phase B0: EXECUTED / INCONCLUSIVE FOR API TRUST.** The accepted Option B spec remains the governing spec context. A separately authorized B0 run halted with `STOP_API_LOCAL_MISMATCH` (`api_count=1747`, `local_count=8`, `matched_count=0`, `mismatch_count=1755`; first mismatches were `API_ONLY`). A follow-up local-only diagnostic then halted with `STOP_API_ARTIFACT_MISSING` because only `option_b_b0_manifest.json` existed under `artifacts/named_binary_probe/price_source_option_b_b0/`; no persisted API rows, mismatch rows, reconciliation JSON, or by-condition CSV were available for offline temporal-overlap analysis. Therefore B0 did **not** establish Data API `/trades` mechanical trust and **blocks B1**, but it does **not** cleanly distinguish API semantic mismatch from stale/incomplete local parquet. Local staleness/incompleteness remains plausible but unverified. Do **not** close Option B as a clean API semantic failure on this evidence. No rerun, fresh Polymarket download, B1, full Pass 1, S2, P1/P2/P3, probe, scoring, backfill, wallet/OrdersMatched/`log_index`/PnL, or gate change is authorized. **P1 remains BLOCKED** on the continued absence of an accepted per-side/token-identity price source. `named_binary_probe_blocked` stays `true`, not flipped. Handoff: `HANDOFF_orchestrator_option_b_b0_RESULT.md`.

- **Option B B0 Failure Diagnostic spec: ACCEPTED / SPEC ONLY.** `SPEC_option_b_b0_failure_diagnostic.md` is the accepted design for a *possible* corrected B0 diagnostic (rerun not authorized by acceptance). Root defect named: the original B0 runner evaluated `STOP_API_LOCAL_MISMATCH` before persisting evidence, destroying the rows needed to diagnose the halt. The spec requires persist-before-halt ordering (D1) with incremental writes; full API/local/mismatch ledgers (D2–D4); per-condition temporal bounds and pagination status (D5–D8); and offline-recomputable overlap classifications (D9). Time-bounded reconciliation safety locked at acceptance: `τ = 120 seconds`, `σ = 24 hours`. The locked `OVERLAP_MATCHED` clean bar: all in-window API and local rows paired by full composite identity, zero `API_ONLY` in-window, zero `LOCAL_ONLY` in-window, zero `TX_HASH_AMBIGUOUS`, pagination complete, artifacts complete and offline-recomputable. The spec authorizes nothing further; a corrected run would require separate explicit second authorization. B1, P1, all downstream remain blocked. `named_binary_probe_blocked` stays `true`. Handoff: `HANDOFF_orchestrator_option_b_b0_failure_diagnostic.md`.
