...
### Option B spec-only review: ACCEPTED / SPEC ONLY — no execution follows from this
...
**What this does and does not authorize.** The spec sets out a bounded two-phase (B0/B1) test design and a set of falsification-minded reconciliation/pagination/typed-parsing/precision requirements (R1–R7) — as **requirements text**, not code. Acceptance of the spec text authorizes **none** of: Phase B0 execution, Phase B1, a full Pass 1, an S2 build, implementation, network/API calls, data runs, P1/P2/P3 continuation, probe execution, scoring, backfill, or any wallet/OrdersMatched/`log_index`/PnL work. Each of those remains a **separate, explicit, in-chat authorization** — this mirrors the discipline already applied to S1 and S1-ALT.

**Disposition of the earlier draft Phase B0 code.** Before canonical project context could be read in the authoring chat thread, a Phase B0 implementation (code + tests) was drafted and iterated across several turns. Once canonical context was read, that work was found to be ahead of authorization — the canonical branch state permitted only a spec-only review, not implementation. That draft code is **draft/reference only, not an accepted artifact**, and must not be committed or treated as authorized implementation. If a future, separately authorized Phase B0 run is approved after this spec's pinning, that draft may be used as a starting reference only, and must be re-validated against the canonical `Store` contract and this spec's R1–R7 requirements before any execution — it should not be assumed correct as-is.

**Standing consequence.** P1 remains **BLOCKED** on the continued absence of an accepted per-side/token-identity price source. `named_binary_probe_blocked` stays `true`.
...
- Option B (Data API `/trades`) spec-only review (`SPEC_price_source_option_b_data_api_review.md`, ACCEPTED / SPEC ONLY). Do not re-litigate the accepted spec text or its two applied wording patches (§8, §1.1). Authorizes no B0/B1/full Pass 1/S2/P1/P2/P3/probe/scoring/backfill; see the settled-decision section above for detail.
