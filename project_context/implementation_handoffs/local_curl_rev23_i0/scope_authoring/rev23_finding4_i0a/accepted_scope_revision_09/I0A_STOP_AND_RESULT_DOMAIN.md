# I0A Stop and Result Domains — Revision 09
...
This Revision 09 amendment package does not independently evaluate, activate, revoke, or modify the separate canonical implementation-authoring package at source-gated HEAD `1e963bb6e8387aff071d697a416fa558956e571e`. It authorizes no implementation action.

## A.1 Private descriptor-set invariant result domain

These private codes are not `I0AResultCode` values and carry no assurance tuple. The unchanged private input can decide exactly four failures plus success, in this order:

| Order | Private code | Exact decidable trigger | Payload |
|---:|---|---|---|
| 1 | `PRIVATE_DESCRIPTOR_SET_ROLE_CARDINALITY_INVALID` | observed role counts differ from `expected_role_counts` | `summaries=null` |
| 2 | `PRIVATE_DESCRIPTOR_SET_ORDINAL_SEQUENCE_INVALID` | summary ordinals are not exact `0..n-1` in tuple order | `summaries=null` |
| 3 | `PRIVATE_DESCRIPTOR_SET_CANONICAL_TARGET_DUPLICATE` | two summaries contain the same canonical target path | `summaries=null` |
| 4 | `PRIVATE_DESCRIPTOR_SET_SIDECAR_RELATION_INVALID` | a sidecar flag/paired ordinal violates the complete same-unit sidecar relation derivable from summaries | `summaries=null` |
| 5 | `PRIVATE_DESCRIPTOR_SET_INVARIANTS_VALID` | every prior private predicate is false | exact non-null ordinal-order summary tuple |

Logical-hash nullability is not a private outcome; it is a public single-descriptor validation. Same-ordinal partition-entry binding is not a private outcome; it is a public descriptor-set validation performed before the private reducer.
...
Revision 09 does not add, remove, reinterpret, or emit a persisted `REV23_STOP_CODE`. Persisted canonical stops remain governed solely by the accepted Revision 23 contract.
