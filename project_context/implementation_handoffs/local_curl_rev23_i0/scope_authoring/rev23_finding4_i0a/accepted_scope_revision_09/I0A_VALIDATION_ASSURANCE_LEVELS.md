# I0A Validation Assurance Levels — Revision 09
...
8. `_validate_descriptor_set_invariants` decides only role cardinality, ordinal sequence, canonical-target uniqueness, and sidecar relation from its unchanged closed input. Logical-hash nullability is decided by `validate_prepared_object_descriptor`; same-ordinal partition-entry binding is decided by `validate_prepared_descriptor_set`.
9. A5, A7, and A8 remain unreachable.
