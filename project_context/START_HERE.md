...
The C1A-F1 bounded canary has executed and is accepted as reviewable mixed coverage/trust evidence only. Any next Option C step must be separately authorized and should begin as review/spec-only. No further C1A-F1 run, additional canary, C1B full sampled coverage, C2 reusable/production implementation, P1/P2/P3/probe, scoring, backfill, wallet/OrdersMatched/`log_index`/PnL, cap change, row truncation, SQL modification, or price artifact is authorized by the C1A-F1 result.
...
No C1A-F1 rerun/additional canary, C1B full sampled coverage, C2 reusable/production implementation, P1/P2/P3 continuation, probe execution, cap increase, row truncation, SQL modification, price artifact, or side synthesis follows from the accepted C1A-F1 result. Do not mark `C1_CANARY_DESIGN_CLEAR`; the observed outcome is `C1_CANARY_EXECUTED_NEEDS_REVIEW` and the evidence remains mixed.
